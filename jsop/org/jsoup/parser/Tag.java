...
/**
 * HTML Tag specifications. This is a very simplistic model without the full
 * expressiveness as the DTD, but it should capture most of what we need to know
 * to intelligently parse a doc.
 * 
...
public class Tag {
	...
	private boolean optionalClosing = false; // If tag is open, and another
												// seen, close previous tag
	private boolean empty = false; // can hold nothing; e.g. img
	private boolean preserveWhitespace = false; // for pre, textarea, script etc
	private List<Tag> ancestors; // elements must be a descendant of one of
									// these ancestors
	private Tag parent; // if not null, elements must be a direct child of
						// parent
	...
	boolean canContain(Tag child) {
		...
		// head can only contain a few. if more than head in here, modify to
		// have a list of valids
		// TODO: (could solve this with walk for ancestor)
		if (this.tagName.equals("head")) {
			if (child.tagName.equals("base") || child.tagName.equals("script")
					|| child.tagName.equals("noscript")
					|| child.tagName.equals("link")
					|| child.tagName.equals("meta")
					|| child.tagName.equals("title")
					|| child.tagName.equals("style")
					|| child.tagName.equals("object")) {
				return true;
			}
			return false;
		}
		...
		if (this.requiresSpecificParent()
				&& this.getImplicitParent().equals(child))
			return false;
	...
	static {
		...
		// tags are set here in uppercase for legibility, but internally held as
		// lowercase.
		// TODO[must]: incorporate html 5 as appropriate

		// document
		createBlock("HTML").setAncestor(new String[0]); // specific includes not
														// impl
		createBlock("HEAD").setAncestor("HTML"); // specific includes not impl:
													// SCRIPT, STYLE, META,
													// LINK, OBJECT
		...
		// head
		// all ancestors set to (head, body): so implicitly create head, but
		// allow in body
		...
		createBlock("LINK").setAncestor("HEAD", "BODY").setEmpty(); // only
																	// within
																	// head
		createInline("OBJECT").setAncestor("HEAD", "BODY"); // flow
															// (block/inline) or
															// param
		...
		createInline("MAP"); // map is defined as inline, but can hold block
								// (what?) or area. Seldom used so NBD.
		createInline("Q");
		createInline("SUB");
		createInline("SUP");
		createInline("SPAN");
		...
		// things past this point aren't really blocks or inline. I'm using them
		// because they can hold block or inline,
		// but per the spec, only specific elements can hold this. if this
		// becomes a real-world parsing problem,
		// will need to have another non block/inline type, and explicit include
		// & exclude rules. should be right though
		...
		createInline("SELECT").setAncestor("FORM"); // just contain optgroup or
													// option
		createInline("TEXTAREA").setAncestor("FORM").setContainDataOnly();
		createInline("LABEL").setAncestor("FORM").setOptionalClosing(); // not
																		// self
		createInline("BUTTON").setAncestor("FORM"); // bunch of excludes not
													// defined
		...
		createBlock("DL");
		createInline("DT").setParent("DL").setOptionalClosing(); // only within
																	// DL.
		createInline("DD").setParent("DL").setOptionalClosing(); // only within
																	// DL.

		createBlock("LI").setAncestor("UL", "OL").setOptionalClosing(); // only
																		// within
																		// OL or
																		// UL.

		// tables
		createBlock("TABLE"); // specific list of only includes (tr, td, thead
								// etc) not implemented
		...
		createBlock("TBODY").setParent("TABLE").setOptionalClosing(); // optional
																		// /
																		// implicit
																		// open
																		// too.
																		// just
																		// TR
		createBlock("COLGROUP").setParent("TABLE").setOptionalClosing(); // just
																			// COL
		createBlock("COL").setParent("COLGROUP").setEmpty();
		createBlock("TR").setParent("TABLE").setOptionalClosing(); // just TH,
																	// TD
