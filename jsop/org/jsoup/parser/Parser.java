...
/**
 * Parses HTML into a {@link Document}. Generally best to use one of the more
...
public class Parser {
	...
	/**
	 * Parse a fragment of HTML into the {@code body} of a Document, with
	 * relaxed parsing enabled. Relaxed, in this context, means that implicit
	 * tags are not automatically created when missing.
	 * 
	 * @param bodyHtml
	 *            fragment of HTML
	 * @param baseUri
	...
	private void parseStartTag() {
		...
		if (tagName.length() == 0) { // doesn't look like a start tag after all;
										// put < back on stack and handle as
										// text
			tq.addFirst("&lt;");
			parseTextNode();
			return;
		...
		// pc data only tags (textarea, script): chomp to end tag, add content
		// as text node
		if (tag.isData()) {
			...
			if (tag.equals(titleTag) || tag.equals(textareaTag)) // want to show
				...
				dataNode = new DataNode(data, baseUri); // data not encoded but
														// raw (for " in script)
		...
		if (child.tagName().equals("base")) {
			...
			if (href.length() != 0) { // ignore <base target> etc
				baseUri = href;
				doc.setBaseUri(href); // set on the doc so
										// doc.createElement(Tag) will get
										// updated base
			}
		}
	}
	...
	private Attribute parseAttribute() {
		...
		if (tq.matchChomp("=")) {
			...
			if (tq.matchChomp(SQ)) {
				...
				// no ' or " to look for, so scan to end tag or space (or end of
				// stream)
				while (!tq.matchesAny("<", "/>", ">")
						&& !tq.matchesWhitespace() && !tq.isEmpty()) {
	...
	private void parseCdata() {
		...
		TextNode textNode = new TextNode(rawText, baseUri); // constructor does
															// not escape
		last().appendChild(textNode);
	}
	...
	private Element popStackToClose(Tag tag) {
		// first check to see if stack contains this tag; if so pop to there,
		// otherwise ignore
		int counter = 0;
		...
		for (int i = stack.size() - 1; i > 0; i--) {
			...
			Element el = stack.get(i);
			Tag elTag = el.tag();
			if (elTag.equals(bodyTag) || elTag.equals(htmlTag)) { // once in
																	// body,
																	// don't
																	// close
																	// past body
