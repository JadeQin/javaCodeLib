 ...
 * 
 * <h2>Selector syntax</h2> A selector is a chain of simple selectors, seperated
 ...
 * <td>an Element with attribute ID of "id"</td>
 * <td><code>div#wrap</code>, <code>#logo</code></td>
 * </tr>
 * <tr>
 * <td><code>E.class</code></td>
 * <td>an Element with a class name of "class"</td>
 * <td><code>div.left</code>, <code>.result</code></td>
 * </tr>
 * <tr>
 * <td><code>E[attr]</code></td>
 * <td>an Element with the attribute named "attr"</td>
 * <td><code>a[href]</code>, <code>[title]</code></td>
 * </tr>
 * <tr>
 * <td><code>E[attr=val]</code></td>
 * <td>an Element with the attribute named "attr" and value equal to "val"</td>
 * <td><code>img[width=500]</code>, <code>a[rel=nofollow]</code></td>
 * </tr>
 * <tr>
 * <td><code>E[attr^=val]</code></td>
 * <td>an Element with the attribute named "attr" and value starting with "val"</td>
 * <td><code>a[href^=http:]</code></code></td>
 * </tr>
 * <tr>
 * <td><code>E[attr$=val]</code></td>
 * <td>an Element with the attribute named "attr" and value ending with "val"</td>
 * <td><code>img[src$=.png]</code></td>
 * </tr>
 * <tr>
 * <td><code>E[attr*=val]</code></td>
 * <td>an Element with the attribute named "attr" and value containing "val"</td>
 ...
 * <td>any matching element E, F, or G</td>
 ...
 * </tr>
 * <tr>
 * <td><code>E:lt(<em>n</em>)</code></td>
 * <td>an Element whose sibling index is less than <em>n</em></td>
 * <td><code>td:lt(3)</code> finds the first 2 cells of each row</td>
 * </tr>
 * <tr>
 * <td><code>E:gt(<em>n</em>)</code></td>
 * <td>an Element whose sibling index is greater than <em>n</em></td>
 * <td><code>td:gt(1)</code> finds cells after skipping the first two</td>
 * </tr>
 * <tr>
 * <td><code>E:eq(<em>n</em>)</code></td>
 * <td>an Element whose sibling index is equal to <em>n</em></td>
 * <td><code>td:eq(1)</code> finds the first cell of each row</td>
 * </tr>
 * <tr>
 * <td><code>E:has(<em>selector</em>)</code></td>
 * <td>an Element that contains at least one element matching the
 * <em>selector</em></td>
 * <td><code>div:has(p)</code> finds divs that contain p elements</td>
 * </tr>
 * <tr>
 * <td><code>E:contains(<em>text</em>)</code></td>
 * <td>an Element that contains the specified text. The search is case
...
public class Selector {
	...
	private final LinkedHashSet<Element> elements; // LHS for unique and ordered
													// elements
	...
	private Elements select() {
		...
		if (tq.matchesAny(combinators)) { // if starts with a combinator, use
			...
			addElements(findElements()); // chomp first element matcher off
											// queue
		}
		...
		while (!tq.isEmpty()) {
			...
			if (tq.matchChomp(",")) { // group or
				...
				Elements candidates = findElements(); // take next el, #. etc
														// off queue
	...
	private void combinator(String combinator) {
		tq.consumeWhitespace();
		String subQuery = tq.consumeToAny(combinators); // support multi >
														// childs
	...
	private Elements byAttribute() {
		...
		if (tq.matchChomp("]")) {
			...
			if (tq.matchChomp("="))
				...
				return root
						.getElementsByAttributeValueNot(key, tq.chompTo("]"));

			else if (tq.matchChomp("^="))
				return root.getElementsByAttributeValueStarting(key,
						tq.chompTo("]"));

			else if (tq.matchChomp("$="))
				return root.getElementsByAttributeValueEnding(key,
						tq.chompTo("]"));

			else if (tq.matchChomp("*="))
				return root.getElementsByAttributeValueContaining(key,
						tq.chompTo("]"));

			else
				throw new SelectorParseException(
						"Could not parse attribute query " + query);
		}
	}
	...
	private Elements has() {
		...
		return filterForParentsOfDescendants(elements,
				select(subQuery, elements));
	}
	...
	private static Elements filterForChildren(Collection<Element> parents,
	...
	// children or lower descendants. input candidates stemmed from found
	// elements, so are either a descendant
	// or the original element; so check that parent is not child
	private static Elements filterForDescendants(Collection<Element> parents,
			Collection<Element> candidates) {
		Elements children = new Elements();
		CHILD: for (Element c : candidates) {
			for (Element p : parents) {
	...
	private static Elements filterForParentsOfDescendants(
			Collection<Element> parents, Collection<Element> children) {
	...
	// adjacent siblings
	private static Elements filterForAdjacentSiblings(
			Collection<Element> elements, Collection<Element> candidates) {
	...
	// preceeding siblings
	private static Elements filterForGeneralSiblings(
			Collection<Element> elements, Collection<Element> candidates) {
	...
	private static Elements filterForSelf(Collection<Element> parents,
			Collection<Element> candidates) {
		Elements children = new Elements();
		CHILD: for (Element c : candidates) {
			for (Element p : parents) {
